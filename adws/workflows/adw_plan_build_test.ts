...
async function runWorkflow(adwId: string): Promise<boolean> {
  ...
  const logger = createLogger(adwId, "plan_build_test");
  ...
  const testModel =
    process.env.ADW_REVIEW_MODEL ?? "claude-sonnet-4-20250514";
  ...
  logger.info(`Models: plan=${planModel} build=${buildModel} test=${testModel}`);
  ...
  try {
    ...
    const planLog = taggedLogger(logger, STEP_PLAN, {
    ...
    completedSteps++;
    logger.info(
      `${STEP_PLAN} step completed (${completedSteps}/${TOTAL_STEPS})`
    );
    ...
    const buildLog = taggedLogger(logger, STEP_BUILD, {
      logDir: logger.logDir,
      step: STEP_BUILD,
    });
    ...
    completedSteps++;
    logger.info(
      `${STEP_BUILD} step completed (${completedSteps}/${TOTAL_STEPS})`
    );
    ...
    const testLog = taggedLogger(logger, STEP_TEST, {
      logDir: logger.logDir,
      step: STEP_TEST,
    });
    ...
    logger.info(`\n${"═".repeat(60)}`);
    logger.info(
      `  WORKFLOW COMPLETE — ${Math.round((Date.now() - startTime) / 1000)}s`
    );
...
if (import.meta.main) {
  ...
  if (!adwId) {
    console.error("Usage: bun run adw_plan_build_test.ts --adw-id <id>");
    process.exit(1);
  }

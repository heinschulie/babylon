 ...
 *
 * Data flow:
 *   1. Static topics from ADW_PROMPT + auto-discovered apps/ dirs → parallel research → docs/*.md
 ...
 *
 * Usage:
 *   ADW_PROMPT="codebase-structure,runtime,deployment,backend,billing,auth" \
...
async function runWorkflow(adwId: string, issueNumber?: string): Promise<boolean> {
  ...
  const staticTopics = topicsRaw.split(",").map((t) => t.trim()).filter(Boolean);
  if (staticTopics.length === 0) {
    logger.error("No topics parsed from ADW_PROMPT");
    return false;
  }
  ...
  try {
    ...
    for (const appName of appNames) {
      ...
      if (appDoc) {
        ...
        logger.warn(`No research doc for app "${appName}" — skipping ${APPS_DIR}/${appName}/README.md`);
      }
    }

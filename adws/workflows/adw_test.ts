...
function formatTestResultsComment(results: TestResult[], passed: number, failed: number): string {
...
async function runWorkflow(adwId: string, issueNumber?: string, skipE2e = false): Promise<boolean> {
	...
	// Create comment functions (before try so catch can access them)
	...
	try {
		...
		logger.info(`\n${'═'.repeat(60)}`);
		logger.info(
			`  WORKFLOW ${ok ? 'COMPLETE' : 'FAILED'} — ${fmtDuration(Date.now() - startTime)}`
		);
